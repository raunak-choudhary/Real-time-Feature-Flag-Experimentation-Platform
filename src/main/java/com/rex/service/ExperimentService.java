...
/**
 * Service class for A/B Testing Experiment management operations. Provides comprehensive business
 * logic for experiment lifecycle management, user cohort assignment, and statistical analysis.
 *
 * <p>Key Features: - A/B testing experiment lifecycle management - Sophisticated user cohort
 * assignment algorithms - Real-time experiment performance tracking - Statistical significance
 * monitoring - Multi-variant experiment support
 */
@Service
@Transactional
public class ExperimentService {
  ...
  public ExperimentService(
      ExperimentRepository experimentRepository, UserCohortRepository userCohortRepository) {
  ...
  public Experiment createExperiment(
    ...
    Experiment experiment =
        new Experiment(
            name, description, trafficPercentage, controlVariantName, testVariantName, createdBy);
  ...
  public Experiment createExperiment(
      ...
      String createdBy) {
    logger.info(
        "Creating comprehensive experiment: {} with target sample size: {}",
        name,
        minimumSampleSize);
  ...
  public List<Experiment> getRunningExperiments() {
    ...
    return experimentRepository.findByStatusOrderByStartDateDesc(
  ...
  public Experiment startExperiment(Long id) {
    ...
    if (!experiment.canBeStarted()) {
      throw new IllegalStateException(
          "Experiment cannot be started in current status: " + experiment.getStatus());
    }
  ...
  /**
   * Assign user to experiment cohort using hash-based algorithm. This ensures consistent assignment
  ...
  public UserCohort assignUserToExperiment(String userId, Long experimentId, String sessionId) {
    ...
    Experiment experiment =
        experimentRepository
            ...
            .orElseThrow(
                () ->
                    new IllegalArgumentException("Experiment not found with ID: " + experimentId));
    ...
    Optional<UserCohort> existingAssignment =
        userCohortRepository.findByUserIdAndExperimentId(userId, experimentId);
    ...
    String variantName =
        cohortType == UserCohort.CohortType.CONTROL
    ...
    logger.info(
        "Successfully assigned user '{}' to experiment '{}' as {} ({})",
        userId,
        experiment.getName(),
        cohortType,
        variantName);
  ...
  public UserCohort assignUserToExperiment(
      String userId,
      Long experimentId,
      UserCohort.AssignmentMethod assignmentMethod,
      UserCohort.CohortType forcedCohortType) {
    logger.info(
        "Manually assigning user '{}' to experiment ID: {} as {}",
        userId,
        experimentId,
        forcedCohortType);
    ...
    String variantName =
        forcedCohortType == UserCohort.CohortType.CONTROL
            ? experiment.getControlVariantName()
            : experiment.getTestVariantName();

    UserCohort cohort =
        new UserCohort(userId, experiment, forcedCohortType, variantName, assignmentMethod);
    cohort.setEnvironment(experiment.getEnvironment());
  ...
  private UserCohort createExcludedCohort(String userId, Experiment experiment, String sessionId) {
    UserCohort cohort =
        new UserCohort(userId, sessionId, experiment, UserCohort.CohortType.EXCLUDED, "excluded");
    cohort.setAssignmentMethod(UserCohort.AssignmentMethod.HASH_BASED);
    cohort.setEnvironment(experiment.getEnvironment());
    return userCohortRepository.save(cohort);
  }
