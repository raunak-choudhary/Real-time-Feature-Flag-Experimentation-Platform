...
/**
 * Service class for Metrics and Analytics operations. Provides comprehensive event tracking,
 * analytics calculation, and real-time metrics aggregation for feature flags and experiments.
 *
 * <p>Key Features: - Event tracking for flags and experiments - Real-time analytics and aggregation
 * - Performance metrics monitoring - Dashboard data preparation - Conversion tracking and funnel
 * analysis
 */
@Service
@Transactional
public class MetricsService {
  ...
  public Metrics trackFlagExposure(
      String userId,
  ...
  public Metrics trackFlagToggle(
      String userId,
      FeatureFlag featureFlag,
      boolean newState,
      String environment,
      String triggeredBy) {
    logger.info(
        "Tracking flag toggle: {} -> {} by user: {}", featureFlag.getName(), newState, triggeredBy);

    Metrics.EventType eventType =
        newState ? Metrics.EventType.FLAG_ENABLED : Metrics.EventType.FLAG_DISABLED;
    ...
    metrics.setProperties(
        String.format("{\"triggered_by\":\"%s\",\"new_state\":%b}", triggeredBy, newState));

    return metricsRepository.save(metrics);
  }

  /** Track feature flag usage with context. */
  public Metrics trackFlagUsage(
      String userId,
      FeatureFlag featureFlag,
      String context,
      String sessionId,
  ...
  public Metrics trackExperimentExposure(
      String userId,
      ...
      String pageUrl) {
    logger.debug(
        "Tracking experiment exposure: {} variant: {} for user: {}",
        experiment.getName(),
        variantName,
        userId);

    Metrics metrics =
        new Metrics(userId, experiment, Metrics.EventType.EXPERIMENT_EXPOSURE, variantName);
    metrics.setSessionId(sessionId);
    metrics.setEnvironment(environment);
    metrics.setUserAgent(userAgent);
    metrics.setPageUrl(pageUrl);
  ...
  public Metrics trackExperimentAssignment(
      String userId,
      Experiment experiment,
      String variantName,
      ...
      String environment) {
    logger.info(
        "Tracking experiment assignment: {} variant: {} for user: {} via: {}",
        experiment.getName(),
        variantName,
        userId,
        assignmentMethod);
    ...
    Metrics metrics =
        new Metrics(userId, experiment, Metrics.EventType.EXPERIMENT_ASSIGNMENT, variantName);
    metrics.setEnvironment(environment);
  ...
  public Metrics trackConversion(
      String userId,
      Experiment experiment,
      String variantName,
      ...
      String environment) {
    logger.info(
        "Tracking conversion: {} variant: {} value: {} for user: {}",
        experiment.getName(),
        variantName,
        conversionValue,
        userId);
    ...
    Metrics metrics =
        new Metrics(userId, experiment, Metrics.EventType.CONVERSION, variantName, conversionValue);
    metrics.setSessionId(sessionId);
    metrics.setEnvironment(environment);
  ...
  public Metrics trackPurchase(
      ...
      String environment) {
    logger.info(
        "Tracking purchase: {} variant: {} revenue: {} for user: {}",
        experiment.getName(),
        variantName,
        revenue,
        userId);
  ...
  /** Track error event. */
  public Metrics trackError(
      String userId,
      String errorMessage,
      String pageUrl,
      String sessionId,
      String environment,
      FeatureFlag relatedFlag) {
  ...
  public Metrics trackPerformance(
      String userId,
      String metricName,
      Double value,
      Long durationMs,
      ...
      String pageUrl) {
    logger.debug(
        "Tracking performance: {} = {} ({}ms) for user: {}", metricName, value, durationMs, userId);

    Metrics.EventType eventType =
        metricName.contains("load")
            ? Metrics.EventType.LOAD_TIME
            : Metrics.EventType.API_RESPONSE_TIME;
  ...
  public Map<String, Object> getExperimentPerformanceSummary(Long experimentId) {
    ...
    for (Object[] row : rawData) {
      ...
      double conversionRate =
          totalEvents > 0 ? (conversions.doubleValue() / totalEvents.doubleValue()) * 100 : 0;
  ...
  public Map<String, Object> getFeatureFlagUsageSummary(Long flagId) {
    ...
    if (rawData.isEmpty()) {
      return Map.of(
          "totalExposures", 0L, "uniqueUsers", 0L, "enabledEvents", 0L, "disabledEvents", 0L);
    }
  ...
  public Map<String, Map<String, Long>> getConversionFunnelSummary(Long experimentId) {
    ...
    for (Object[] row : rawData) {
      ...
      funnelData.computeIfAbsent(variantName, k -> new HashMap<>()).put(eventType, eventCount);
    }
  ...
  public Map<String, Object> getDashboardOverview(String environment) {
    ...
    long totalEvents24h = hourlyData.stream().mapToLong(row -> (Long) row[1]).sum();
    overview.put("totalEvents24h", totalEvents24h);

    // Unique users in last 24 hours
    long uniqueUsers24h = hourlyData.stream().mapToLong(row -> (Long) row[2]).sum();
    overview.put("uniqueUsers24h", uniqueUsers24h);

    // Error count in last 24 hours
    List<Object[]> errorData = getErrorMetrics(last24Hours);
    long errorCount24h = errorData.stream().mapToLong(row -> (Long) row[1]).sum();
  ...
  public void cleanupOldMetrics(LocalDateTime cutoffDate) {
    ...
    List<Metrics> oldMetrics =
        metricsRepository.findByTimestampBetween(LocalDateTime.of(2020, 1, 1, 0, 0), cutoffDate);
