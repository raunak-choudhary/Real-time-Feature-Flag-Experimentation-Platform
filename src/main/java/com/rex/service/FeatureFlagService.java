...
/**
 * Service class for Feature Flag management operations. Provides comprehensive business logic for
 * real-time feature flag management, including CRUD operations, toggle functionality, and rollout
 * management.
 *
 * <p>Key Features: - Real-time flag toggling without deployment - Gradual rollout percentage
 * management - Environment-specific flag handling - Validation and error handling - Audit trail
 * support
 */
@Service
@Transactional
public class FeatureFlagService {
  ...
  /** Create a new feature flag with validation. Ensures flag names are unique per environment. */
  public FeatureFlag createFeatureFlag(
      String name, String description, String environment, String createdBy) {
  ...
  public FeatureFlag createFeatureFlag(
      ...
      String createdBy) {
    logger.info(
        "Creating feature flag: {} with status: {} and rollout: {}%",
        name, status, rolloutPercentage);
  ...
  public List<FeatureFlag> getActiveFlagsByEnvironment(String environment) {
    ...
    return featureFlagRepository.findByStatusAndEnvironment(
        FeatureFlag.FlagStatus.ACTIVE, environment);
  }

  /** Update feature flag. */
  public FeatureFlag updateFeatureFlag(
      Long id, String name, String description, String environment) {
    ...
    FeatureFlag flag =
        featureFlagRepository
            .findById(id)
            .orElseThrow(
                () -> new IllegalArgumentException("Feature flag not found with ID: " + id));
  ...
  /**
   * Toggle feature flag state (real-time operation). This is the core functionality for instant
   * feature toggling.
   */
  public FeatureFlag toggleFlag(Long id) {
    ...
    logger.info(
        "Successfully toggled feature flag '{}' from {} to {}",
        flag.getName(),
        wasEnabled,
  ...
  /**
   * Evaluate if a flag should be enabled for a specific user. Considers rollout percentage and user
   * hash for consistent assignment.
   */
  public boolean isFlagEnabledForUser(String flagName, String userId, String environment) {
    logger.debug(
        "Evaluating flag '{}' for user '{}' in environment '{}'", flagName, userId, environment);
    ...
    if (!flag.getEnvironment().equals(environment)) {
      logger.debug(
          "Flag '{}' environment mismatch. Expected: {}, Got: {}",
          flagName,
          flag.getEnvironment(),
          environment);
      return false;
    ...
    logger.debug(
        "Flag '{}' for user '{}': percentile={}, rollout={}%, enabled={}",
  ...
  public List<FeatureFlag> enableFlags(List<Long> flagIds) {
    ...
    return flagIds.stream().map(this::enableFlag).toList();
  }
  ...
  public List<FeatureFlag> disableFlags(List<Long> flagIds) {
    ...
    return flagIds.stream().map(this::disableFlag).toList();
  }
  ...
  public List<FeatureFlag> updateRolloutForFlags(List<Long> flagIds, Integer percentage) {
    ...
    return flagIds.stream().map(id -> updateRolloutPercentage(id, percentage)).toList();
  }
  ...
  private void validateFlagName(String name) {
    ...
    if (!name.matches("^[a-zA-Z0-9_-]+$")) {
      throw new IllegalArgumentException(
          "Flag name can only contain letters, numbers, underscores, and hyphens");
    }
  }
  ...
  /**
   * Calculate user percentile for consistent flag assignment. Uses hash-based algorithm to ensure
