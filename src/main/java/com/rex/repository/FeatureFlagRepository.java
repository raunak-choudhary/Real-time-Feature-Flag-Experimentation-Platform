...
/**
 * Repository interface for FeatureFlag entity operations. Provides CRUD operations and custom
 * queries for feature flag management. VERIFIED: All enum references and query syntax are correct
 * for compilation.
 */
@Repository
public interface FeatureFlagRepository extends JpaRepository<FeatureFlag, Long> {
  ...
  @Query(
      "SELECT ff FROM FeatureFlag ff WHERE ff.rolloutPercentage > :percentage AND ff.enabled = true")
  ...
  @Query(
      "SELECT ff FROM FeatureFlag ff WHERE ff.updatedAt >= :sinceDate ORDER BY ff.updatedAt DESC")
  List<FeatureFlag> findRecentlyUpdated(@Param("sinceDate") java.time.LocalDateTime sinceDate);

  /**
   * Custom query to find feature flags suitable for a user based on rollout percentage. This is
  ...
  List<FeatureFlag> findAvailableFlagsForUser(
      @Param("environment") String environment, @Param("userPercentile") Integer userPercentile);
}
