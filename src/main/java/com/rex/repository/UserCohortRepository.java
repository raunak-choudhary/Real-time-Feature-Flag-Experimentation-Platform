...
/**
 * Repository interface for UserCohort entity operations. Provides CRUD operations and custom
 * queries for user experiment assignment management. FIXED: Corrected relationship navigation for
 * experiment entities.
 */
@Repository
public interface UserCohortRepository extends JpaRepository<UserCohort, Long> {
  ...
  @Query(
      "SELECT uc FROM UserCohort uc WHERE uc.userId = :userId AND uc.experiment.id = :experimentId")
  Optional<UserCohort> findByUserIdAndExperimentId(
      @Param("userId") String userId, @Param("experimentId") Long experimentId);
  ...
  @Query(
      "SELECT uc FROM UserCohort uc WHERE uc.experiment.id = :experimentId AND uc.cohortType = :cohortType")
  List<UserCohort> findByExperimentIdAndCohortType(
      @Param("experimentId") Long experimentId,
      @Param("cohortType") UserCohort.CohortType cohortType);

  /** Find assignments by experiment and variant name. */
  @Query(
      "SELECT uc FROM UserCohort uc WHERE uc.experiment.id = :experimentId AND uc.variantName = :variantName")
  List<UserCohort> findByExperimentIdAndVariantName(
  ...
  @Query(
      "SELECT COUNT(uc) FROM UserCohort uc WHERE uc.experiment.id = :experimentId AND uc.cohortType = :cohortType")
  long countByExperimentIdAndCohortType(
      @Param("experimentId") Long experimentId,
      @Param("cohortType") UserCohort.CohortType cohortType);

  /** Count active assignments by experiment. */
  @Query(
      "SELECT COUNT(uc) FROM UserCohort uc WHERE uc.experiment.id = :experimentId AND uc.isActive = true")
  long countByExperimentIdAndIsActiveTrue(@Param("experimentId") Long experimentId);

  /** Check if user is assigned to experiment. */
  @Query(
      "SELECT COUNT(uc) > 0 FROM UserCohort uc WHERE uc.userId = :userId AND uc.experiment.id = :experimentId")
  boolean existsByUserIdAndExperimentId(
      @Param("userId") String userId, @Param("experimentId") Long experimentId);

  /** Find assignments with high exposure count (engaged users). */
  @Query(
      "SELECT uc FROM UserCohort uc WHERE uc.exposureCount >= :minExposure ORDER BY uc.exposureCount DESC")
  ...
  @Query(
      """
        ...
        """)
  List<UserCohort> findAssignmentsInDateRange(
  ...
  @Query(
      """
        ...
        ORDER BY uc.assignedAt DESC
        """)
  List<UserCohort> findRecentAssignments(
      @Param("experimentId") Long experimentId, @Param("sinceDate") LocalDateTime sinceDate);
}
